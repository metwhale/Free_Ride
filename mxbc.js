...
let s_mxbc = ($.isNode() ? process.env.s_mxbc : $.getdata("s_mxbc")) || ""
let s_mxbcArr = [];
let data = '';
let msg = '';
...
!(async () => {
    if (typeof $request !== "undefined") {
        ...
        if (!(await Envs()))
            ...
            log(`\n============ 微信公众号：AutMan福利社 ============`)
            log(`\n=================== 共找到 ${s_mxbcArr.length} 个账号 ===================`)
            if (debug) {
                log(`【debug】 这是你的全部账号数组:\n ${s_mxbcArr}`);
            }
            for (let index = 0; index < s_mxbcArr.length; index++) {

                let num = index + 1
                addNotifyStr(`\n==== 开始【第 ${num} 个账号】====\n`, true)

                s_mxbc = s_mxbcArr[index];            
...
async function Envs() {
    if (s_mxbc) {
        if (s_mxbc.indexOf("@") != -1) {
            s_mxbc.split("@").forEach((item) => {

                s_mxbcArr.push(item);
            });
        } else if (s_mxbc.indexOf("\n") != -1) {
            s_mxbc.split("\n").forEach((item) => {
                s_mxbcArr.push(item);
            });
        } else {
            s_mxbcArr.push(s_mxbc);
        }
    } else {
        log(`\n 【${$.name}】：未填写变量 s_mxbc`)
        return;
